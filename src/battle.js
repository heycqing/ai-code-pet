...
import { getSprite, SPRITE_WIDTH } from './pixelRenderer.js';
...
// Pad sprite lines to uniform height and terminal width for side-by-side layout
function padLines(lines, height, termWidth) {
  const out = [...lines];
  while (out.length < height) out.unshift('');
  return out.map(l => {
    const visible = l.replace(/\x1B\[[0-9;]*m/g, '');
    return l + ' '.repeat(Math.max(0, termWidth - visible.length));
  });
}
...
// Battle UI
// ─────────────────────────────────────────────
const COL = SPRITE_WIDTH + 4; // terminal chars per sprite column (24 + 4 padding)
...
function renderBattle(player, enemy, log) {
  ...
  const pFrames = getSprite(player.type, player.stageIndex, 'idle');
  const eFrames = getSprite(enemy.type,  enemy.stageIndex,  'idle');
  const artH    = Math.max(pFrames.length, eFrames.length);

  const pl = padLines(pFrames, artH, COL);
  const en = padLines(eFrames, artH, COL);

  // HP headers
  console.log(
    `  ${C.player(player.name.padEnd(18))}      ${C.enemy(enemy.name)}`
  );
  console.log(
  ...
  for (let i = 0; i < artH; i++) {
    console.log(`  ${pl[i]}      ${en[i]}`);
  }
  console.log();

  // Battle log (last 3 lines, always 3 lines to prevent layout shift)
...
function renderBattleMenu(specialName, canSpecial) {
  ...
  console.log(`  ${C.player('[A]')} 攻击   ${sp}   ${C.defend('[D]')} 防御   ${C.flee('[F]')} 逃跑`);
}
...
function doEnemyTurn(enemy, player, log) {
  ...
  if (roll < 0.15) {
    ...
    player.hp = Math.max(0, player.hp - dmg);
    log.push(`${C.enemy(enemy.name)} 攻击！造成 ${C.damage(dmg + ' 伤害')}`);
  }
}
...
export async function startBattle(pet) {
  ...
  log.push(playerFirst ? C.player(`${player.name} 速度更快，先手出击！`) : C.enemy(`${enemy.name} 抢先出手！`));
